...
const cartLogo = <svg className="fill-current" viewBox="-1 0 37 32" height="18" width="18" ><path d="M4.438 0l-2.598 5.11-1.84 26.124h34.909l-1.906-26.124-2.597-5.11z"></path></svg>

const linkLogoStyle = `flex h-full items-center justify-around relative hover:text-headerHoverColor hover:fill-headerHoverColor`

const linkObj = [
    { name: "Search",  logo: searchLogo },
    { name: "Offers",  logo: offerLogo, className2: " after:content-['NEW'] after:absolute after:text-amber-500 after:font-bold after:top-7 after:-right-5 after:text-[9px]" },
    { name: "Help",    logo: helpLogo },
    { name: "Sign In", path: "/sign-in", logo: signInLogo },
    { name: "Cart",    logo: cartLogo, className2: " hover:stroke-headerHoverColor" },
]
...
const Header = (props) => {
    ...
    return (
        <>
            <div className="w-full h-16 md:h-20 bg-white shadow-[0_15px_40px_-20px_rgba(40,44,63,.15)] fixed top-0 left-0 z-30">
                <div className="w-full md:w-4/5 h-full mx-auto px-4 md:px-0">
                    <div className="flex items-center justify-between w-full h-full">

                        {/* Logo + location */}
                        <div className="flex justify-between items-center text-[14px] h-full leading-[17px] min-w-0">
                            <NavLink className="flex h-3/4 hover:scale-110 transition duration-200 ease-in-out flex-shrink-0" to="/">
                                <img src={SwiggyLogo} className="h-full" alt="Logo" />
                            </NavLink>

                            {!helpPage && (
                                <button
                                    className="ml-4 hidden sm:block"
                                    onClick={() => props.locBarStateFunc(prev => !prev)}
                                >
                                    <div className="flex justify-between items-center h-full [&>*:nth-child(1)]:text-sortByBtnHoverColor [&>*:nth-child(1)]:after:bg-sortByBtnHoverColor [&>*:nth-child(2)]:text-sortByBtnColor [&>*:nth-child(3)]:text-orange-400 [&>*:nth-child(1)]:hover:text-headerHoverColor [&>*:nth-child(1)]:hover:after:bg-headerHoverColor [&>*:nth-child(2)]:hover:text-[#93959f]">
                                        <p className="font-extrabold ml-2 mr-2 after:content-[' '] after:block after:mt-[2px] after:h-[2px] after:w-full truncate max-w-[80px] lg:max-w-none">{location?.name?.[0]}</p>
                                        <p className="mr-2 h-5 font-normal hidden lg:block truncate max-w-[160px]">{location?.name?.[1]}</p>
                            ...
                            {helpPage && (
                                <p className="text-sm m-4 text-sortByBtnHoverColor font-extrabold">HELP</p>
                            )}
                        </div>

                        {/* Desktop nav */}
                        <ul className="hidden md:flex h-full items-center gap-x-1 lg:gap-x-2 text-[15px] font-semibold [&>*:nth-child(5)]:stroke-ttlRestroHeading [&>*:nth-child(5)]:stroke-2">
                            {linkObj.map((link) => {
                                const to = link.path ?? `/${link.name}`
                                return (
                                    <NavLink
                                        to={to}
                                        key={link.name}
                                        className={({ isActive }) =>
                                            (isActive ? "text-headerHoverColor fill-headerHoverColor " : "text-sortByBtnHoverColor fill-sortByBtnHoverColor ") +
                                            "h-full flex justify-center items-center"
                                        }
                                    >
                                        <li className={`${linkLogoStyle} px-2 lg:px-3 gap-x-1 lg:gap-x-2${link?.className2 ? ' ' + link.className2 : ''}`}>
                                            <span className="relative">
                                                {link.logo}
                                                {link.name === 'Cart' && cartCount > 0 && (
                                                    <span className="absolute -top-2 -right-2 bg-headerHoverColor text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
                        ...
                        {/* Mobile hamburger */}
                        <button
                            className="md:hidden p-2 text-sortByBtnHoverColor"
            ...
            {/* Mobile drawer */}
            {menuOpen && (
                <div className="md:hidden fixed top-16 left-0 right-0 z-30 bg-white shadow-lg border-t border-[#e9e9eb]">
                    {/* location row on mobile */}
                    {!helpPage && (
                        <button
                            className="w-full flex items-center gap-x-3 px-5 py-4 border-b border-[#e9e9eb] text-sortByBtnHoverColor font-semibold text-sm"
                            onClick={() => { props.locBarStateFunc(prev => !prev); setMenuOpen(false) }}
                        >
                            <i className="fa-solid fa-location-dot text-headerHoverColor" />
                            <span className="truncate">{location?.name?.[1] || location?.name?.[0]}</span>
                            <i className="fa-sharp fa-solid fa-angle-down ml-auto text-locationError" />
                        </button>
                    )}
                    {linkObj.map((link) => {
                        const to = link.path ?? `/${link.name}`
                        return (
                            <NavLink
                                ...
                                className={({ isActive }) =>
                                    `flex items-center gap-x-4 px-5 py-4 border-b border-[#e9e9eb] text-sm font-semibold ${isActive ? 'text-headerHoverColor fill-headerHoverColor' : 'text-sortByBtnHoverColor fill-sortByBtnHoverColor'}`
                                }
                            >
                                <span className="w-5 flex-shrink-0">{link.logo}</span>
                                {link.name}
                            </NavLink>
                        )
                    })}
